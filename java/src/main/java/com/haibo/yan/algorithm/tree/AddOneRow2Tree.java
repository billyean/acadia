 ...
 * Given the root of a binary tree, then val v and depth d, you need to add a row of nodes with val v at the given
 ...
 * nodes with val v as N's left subtree root and right subtree root. And N's original left subtree should be the left
 * subtree of the new left subtree root, its original right subtree should be the right subtree of the new right subtree
 * root. If depth d is 1 that means there is no depth d-1 at all, then create a tree node with val v as the new root
